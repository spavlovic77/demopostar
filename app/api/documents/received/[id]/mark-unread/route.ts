...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.log("[v0] Marking transaction as unread:", params.id)

    const response = await fetch(`${ION_AP_BASE_URL}/api/v2/receive-transactions/${params.id}/mark-unread`, {
      method: "POST",
      headers: {
    ...
    console.log("[v0] Mark-unread response status:", response.status)

    if (!response.ok) {
      const errorText = await response.text()
      console.error("[v0] Mark as unread failed:", response.status, errorText)
    ...
    console.error("[v0] Error marking as unread:", error)
